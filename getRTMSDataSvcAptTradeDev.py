...
aptSeqs = {
    "28177-6824":"용현자이크레스트",
    "28177-6035":"인천 SK Sky VIEW",
    "28177-6446":"엘에이치미추홀퍼스트",
}
...
def get_data():
    ...
    filtered_items = [item for item in items if item["aptSeq"] in aptSeqs.keys()]

    # print(json.dumps(filtered_items, indent=4, ensure_ascii=False))
    for item in filtered_items:
        # print(item)
        print(item["aptNm"], item["dealYear"], item["dealMonth"], item["dealDay"], item["aptDong"], item["floor"], item["excluUseAr"], item["dealAmount"], item["dealingGbn"])
